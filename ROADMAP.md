...
- ~~预设断点模板（设计方案 §6）~~ ✅ 已做。`ApkIndex` 沿 APK 内父类链识别 Activity / Application
  子类里自身声明的 `onCreate`，`smaliscope` 的 Web（断点面板「一键断在」按钮）与 MCP
  （`set_breakpoint_template`）都能一键把断点下到这些入口的 dex_pc 0。`TemplateTest` 用 testapp 验证。
- ~~时间线单步时不实时增长~~ ✅ 实际上 `applyState` 每次挂起都已刷新时间线（注释过时了）；
  本次顺手改成「只在时间线面板可见时才重建 DOM」，避免隐藏时每步白建。
- ~~偶发：连开新会话断点等待超时~~ ✅ 定位到根因并修复：`am force-stop` 是异步的，
  紧接着 `set-debug-app -w` + `am start` 偶尔赶在旧进程回收完成之前，新进程没被打上
  「等待调试器」标记、直接跑过断点，表现就是超时。现在 force-stop 后等进程真正消失再继续，
  并检查 `am start` 的输出（它失败时仍以 0 退出，之前被吞成一句超时），失败则退回 monkey。
  模拟器上背靠背 8 次全部命中；样本有限，但修的是一个确凿的竞态，不是盲猜。

## 二期（设计方案 §1.3 划的）

- ~~寄存器写入~~ ✅ 已做。`StackFrame.SetValues`：挂起时改任一类型可推出的寄存器
  （引用型只支持写 null），tag 沿用读时那套，写错会破坏帧所以类型未定的一律拒绝。
  Web 寄存器面板每行有「改」按钮，MCP 加 `write_register` 工具。
  实测因果成立：把 `v1` 改成 5 后步过 `mul-int v2,p1,v1`，`v2` 变成 3×5=15。
- **条件断点 / 表达式求值**：仍未做。条件断点可先做「命中计数 / 寄存器等于某值」这类
  不需要完整表达式引擎的形式。

## 仍未做

- jpackage 只在 macOS 出过 `.dmg`；Windows/Linux 的包要在对应系统各跑一次。
