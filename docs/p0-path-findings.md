...
- **P2**（非 root 重打包 + 重签名）：从「最后手段」升为**调试第三方应用的主路径**。
  ARSCLib 改 `android:debuggable` → apksig 重签名 → 卸载重装。

也就是说，「想调别人的 App」这件事在现代 Android 上**没有免改造的捷径**，
无论有没有 root、无论什么镜像。这一点应当在产品里对用户直说，而不是让他
换了非 Play 镜像之后发现还是不行。
